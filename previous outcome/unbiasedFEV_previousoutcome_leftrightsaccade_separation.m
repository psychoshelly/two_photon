...
save('unbiased_previousoutcome1108.mat','unbiased_previousoutcome1108');

% shadedErrorBar(([1:40]-20)/7.4,mean(unbiased_previousoutcome1108(:,1:40),1),std(unbiased_previousoutcome1108(:,1:40),1)./sqrt(length(unbiased_previousoutcome1108(:,1:40))),'lineProps','r');
% xline(0);
% ylim([0 0.02]);
% 
% 
...
%        unbiased_previousoutcome1108(a,:)=-unbiased_previousoutcome1108(a,:);
%     end
% end
% 
% unbiased_currentoutcome_meanvalue1108=mean(unbiased_previousoutcome1108(:,9:32),2);
...
% unbiased_FEV_sort(i,:)=unbiased_previousoutcome1108(find(s1108==unbiased_currentoutcome1108_sort(i,2)),:);
% end
% 
% h=heatmap(unbiased_FEV_sort(:,1:40),'Colormap',jet);
% grid off;
...
% 
% shadedErrorBar([1:24],mean(unbiased_saccadeleft_previous_wrongcorrect(:,9:32),1),std(unbiased_saccadeleft_previous_wrongcorrect(:,9:32),1)/sqrt(length(unbiased_saccadeleft_previous_wrongcorrect(:,9:32))),'lineProps','b');
% xline(13);
% hold on
% shadedErrorBar([1:24],mean(unbiased_saccaderight_previous_wrongcorrect(:,9:32),1),std(unbiased_saccaderight_previous_wrongcorrect(:,9:32),1)/sqrt(length(unbiased_saccaderight_previous_wrongcorrect(:,9:32))),'lineProps','k');
% legend('left','right');
% 
% 
% significant=[];
% for i=1:size(unbiased_saccadeleft_previous_wrongcorrect(:,9:32),2) 
% %p=friedman([unbiased_saccade_previous_correct(:,i+7) unbiased_saccade_previous_wrong(:,i+7)],1)
% [h,p]=ttest(unbiased_saccadeleft_previous_wrongcorrect(:,i+8),unbiased_saccaderight_previous_wrongcorrect(:,i+8),'Alpha',0.01)
% significant=[significant [h;p]];
% end
