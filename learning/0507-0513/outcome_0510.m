...
bh=mlread('G:\0507-0513\20220510\xiaoliu_7a_SR_associated_220510_02.bhv2');
 a1_left_correct=[];
 a1_right_correct=[];
 a2_left_wrong=[];
 a2_right_wrong=[];
...
for i=1:length(unbiased_current_left)
    ...
    unbiased_currentoutcome_region3_0510(i,j)=mean([unbiased_current_left(i,j),unbiased_current_right(i,j)]);
    end
end
                            
shadedErrorBar(([1:19]-3)/7.4,mean(unbiased_currentoutcome_region3_0510(:,2:20),1),std(unbiased_currentoutcome_region3_0510(:,2:20),1)./sqrt(length(unbiased_currentoutcome_region3_0510(:,2:20))),'lineProps','c');
 xline(0);
 ylim([0 0.06]);
% save('G:\0507-0513\20220510\unbiased_currentoutcome_region3_0510_raw.mat','unbiased_currentoutcome_region3_0510');
% 


% load('F:\region3_same_neurons\0510\proportions_of_cells_encoding_current_trials_outcome_20220510.mat');
% iscell=readNPY('F:\region3_same_neurons\0510\suite2p\plane0\iscell.npy');
% s0510=find(iscell(:,1)==1);
% for i=1:length(M2region3day4)
%     if M2region3day4(i,6)==-1
%        a=find(s0510==M2region3day4(i,5));
%        unbiased_currentoutcome_region3_0510(a,:)=-unbiased_currentoutcome_region3_0510(a,:);
%     end
% end

load('G:\0507-0513\20220510\judge_the_types_of_neurons_on_each_frame_20220510.mat');

for i=1:length(s0510)
    for j=36:size(M2region3day4_difference,2)
        if M2region3day4_difference(i,j)<0
            unbiased_currentoutcome_region3_0510(i,j-34)=-unbiased_currentoutcome_region3_0510(i,j-34);
        end
    end
end
...
unbiased_FEV_sort_region3_0510(i,:)=unbiased_currentoutcome_region3_0510(find(s0510==unbiased_currentoutcome_meanvalue_sort_region3_0507(i,5)),:);
end

h=heatmap(unbiased_FEV_sort_region3_0510(:,2:20),'Colormap',jet);
grid off;
h.ColorLimits=[-0.05 0.05];


% h=heatmap(unbiased_FEV_sort_region3_0510(50:70,2:20),'Colormap',jet);
% grid off;
% h.ColorLimits=[-0.05 0.05];
