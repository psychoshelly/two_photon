%region3_0508
load('G:\0507-0513\20220508\stim_markers_learning_aligned_to_target_off_20220508.mat');
%% correct
prev_t=4;
after_t=16;
...
bh=mlread('G:\0507-0513\20220508\xiaoliu_7a_SR_associated_220508.bhv2');
 a1_left_correct=[];
 a1_right_correct=[];
 a2_left_wrong=[];
 a2_right_wrong=[];
...
for i=1:length(unbiased_current_left)
    ...
    unbiased_currentoutcome_region3_0508(i,j)=mean([unbiased_current_left(i,j),unbiased_current_right(i,j)]);
    end
end

shadedErrorBar(([1:19]-3)/7.4,mean(unbiased_currentoutcome_region3_0508(:,2:20),1),std(unbiased_currentoutcome_region3_0508(:,2:20),1)./sqrt(length(unbiased_currentoutcome_region3_0508(:,2:20))),'lineProps','g');
xline(0);
ylim([0 0.06]);


% save('G:\0507-0513\20220508\unbiased_currentoutcome_region3_0508_raw.mat','unbiased_currentoutcome_region3_0508');
% 


% load('F:\region3_same_neurons\0508\proportions_of_cells_encoding_current_trials_outcome_20220508.mat');
% iscell=readNPY('F:\region3_same_neurons\0508\suite2p\plane0\iscell.npy');
% s0508=find(iscell(:,1)==1);
% for i=1:length(M2region3day2)
%     if M2region3day2(i,6)==-1
%        a=find(s0508==M2region3day2(i,5));
%        unbiased_currentoutcome_region3_0508(a,:)=-unbiased_currentoutcome_region3_0508(a,:);
%     end
% end

load('G:\0507-0513\20220508\judge_the_types_of_neurons_on_each_frame_20220508.mat');

for i=1:length(s0508)
    for j=36:size(M2region3day2_difference,2)
        if M2region3day2_difference(i,j)<0
            unbiased_currentoutcome_region3_0508(i,j-34)=-unbiased_currentoutcome_region3_0508(i,j-34);
        end
    end
end
...
unbiased_FEV_sort_region3_0508(i,:)=unbiased_currentoutcome_region3_0508(find(s0508==unbiased_currentoutcome_meanvalue_sort_region3_0507(i,3)),:);
end

h=heatmap(unbiased_FEV_sort_region3_0508(:,2:20),'Colormap',jet);
grid off;
h.ColorLimits=[-0.05 0.05];


% h=heatmap(unbiased_FEV_sort_region3_0508(50:70,2:20),'Colormap',jet);
% grid off;
% h.ColorLimits=[-0.05 0.05];
